...
export default async function (
  this: any,
  query: string,
  options = { fetchPlayer: false, getMuseum: false, getGarden: false }
) {
  ...
  if (options.fetchPlayer) {
    const uniqueUuids = [...new Set(res.profiles.map((profile: any) => Object.keys(profile.members)).flat())];
    await Promise.all(
      uniqueUuids.map(async (uuid) => {
        const player = await getPlayer.call(this, uuid as string, options as any);
