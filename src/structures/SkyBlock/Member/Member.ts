...
class SkyblockMember {
  ...
  constructor(data: Record<string, any>) {
    ...
    this.skills = new Skills(data.m);
    this.bestiary = getBestiaryLevel(data.m);
    this.slayer = new Slayers(data.m);
    this.crimsonIsle = new CrimsonIsle(data.m);
    this.dungeons = new Dungeons(data.m);
    this.collections = data?.m?.collection || {};
    this.purse = data?.m?.currencies?.coin_purse || 0;
    this.stats = new Stats(data?.m?.player_stats);
    this.pets = data?.m?.pets_data?.pets ? data.m.pets_data.pets.map((pet: any) => new SkyblockPet(pet)) : [];
    this.jacob = new Jacob(data.m);
    this.chocolate = new ChocolateFactory(data.m);
    this.minions = new Minions(data.m?.player_data?.crafted_generators ?? []);
    this.getArmor = async () => {
